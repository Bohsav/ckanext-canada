...
from ckan.types import (Action, AuthFunction, ChainedAction,
                        ChainedAuthFunction, CKANApp, DataDict)
from click import Command
from flask import Blueprint, has_request_context

from ckanext.activity.logic.validators import object_id_validators
from ckanext.canada import auth, cli
...
class CanadaPublicPlugin(p.SingletonPlugin, DefaultTranslation):
    ...
    def update_config(self, config: "CKANConfig"):
        ...
        assert (
            "ckanext.canada:schemas/organization.yaml" in scheming_organization_schemas
        )
    ...
    def group_facets(
        self, facets_dict: "OrderedDict[str, Any]", group_type: str, package_type: str
    ) -> "OrderedDict[str, Any]":
        """Update the facets_dict and return it."""
    ...
    # IAuthFunctions
    def get_auth_functions(self) -> Dict[str, Union[AuthFunction, ChainedAuthFunction]]:
        return {
    ...
    #     return LogExtraMiddleware(app, config, getattr(app, "original_flask_app", None))
    ...
    def _update_canada_citation_map(self, cite_data: DataDict, pkg_dict: DataDict):
        lang = "en"
        try:
            lang = h.lang()
        except RuntimeError:
        ...
        if pkg_dict.get("creator"):
            cite_data["author"].append({"family": pkg_dict["creator"]})
        if contributor := pkg_dict.get("contributor", {}).get(lang):
            cite_data["author"].append({"family": contributor})
        if pkg_dict.get("credit"):
            for e in pkg_dict["credit"]:
                if creditor := e.get("credit_name", {}).get(lang):
                    cite_data["author"].append({"family": creditor})
        if not cite_data["author"]:
            cite_data.pop("author", None)

    # ICiteProcMappings
    def update_dataset_citation_map(
        self, cite_data: DataDict, pkg_dict: DataDict
    ) -> bool:
        cite_data["container_title"] = _(cite_data["container_title"])
        self._update_canada_citation_map(cite_data, pkg_dict)
        return False

    def update_resource_citation_map(
        self, cite_data: DataDict, pkg_dict: DataDict, res_dict: DataDict
    ) -> bool:
        cite_data["container_title"] = _(cite_data["container_title"])
        self._update_canada_citation_map(cite_data, pkg_dict)
        return False
...
class LogExtraMiddleware(object):
    def __init__(self, app: Any, config: "CKANConfig", flask_app: Optional[Any] = None):
    ...
    def _log_support_id(self, e: Exception):
        supportID = (
            "".join(re.findall(r"\d+", socket.gethostname())) + "-" + str(time.time())
        )
    ...
    def __call__(self, environ: Any, start_response: Any) -> Any:
        def _start_response(
            ...
            return start_response(status, response_headers + extra, exc_info)

        return self.app(environ, _start_response)


def _wet_pager_admin_url_generator(
    page: int, partial: Optional[str] = None, **kwargs: Any
) -> str:
    pargs = []
