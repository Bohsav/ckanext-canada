...
from ckan.plugins.toolkit import (NotAuthorized, ObjectNotFound, _, asbool,
                                  aslist, config, g, get_action, h, request)
...
def get_translated_t(data_dict: Dict[str, Any], field: str) -> Tuple[str, bool]:
    """
...
def language_text_t(
    text: Union[Dict[str, Any], str], prefer_lang: Optional[str] = None
...
def catalogue_last_update_date() -> str:
    ...
    q = (
        model.Session.query(Activity.timestamp)
...
def date_format(date_string: Optional[str]) -> Optional[str]:
    if not date_string:
        return None
    try:
        return datetime.datetime.strptime(date_string, "%Y-%m-%d %H:%M:%S").strftime(
            "%Y-%m-%d"
        )
    except ValueError:
        return date_string
...
def adv_search_url() -> str:
    return (
        config.get("ckanext.canada.adv_search_url_fr")
        if h.lang() == "fr"
        else config.get("ckanext.canada.adv_search_url_en")
    )


def adv_search_mlt_root() -> str:
    return (
        "{0}/donneesouvertes/similaire/".format(
            config.get("ckanext.canada.adv_search_url_fr")
        )
        if h.lang() == "fr"
        else "{0}/opendata/similar/".format(
...
def adobe_analytics_creator(
    ...
    if organization:
        ...
        if org_title and " | " in org_title:
            creator = org_title.split(" | ")[1 if h.lang() == "fr" else 0].strip()
        else:
    ...
    if (
        package
        and "org_section" in package
        and h.scheming_language_text(package.get("org_section"))
    ):
    ...
    return Markup(
        '<meta property="dcterms:creator" content="%s" '
...
def resource_view_meta_title(
    ...
    if not is_subtitle:
        return "%s - %s - %s - %s" % (
            html.escape(package_title),
            html.escape(resource_title),
            html.escape(view_title),
            html.escape(_(g.site_title)),
        )
    return Markup(
        "%s - %s - %s"
        % (
...
def get_pd_datatable(
    resource_name: str, resource_id: str, owner_org: str, dataset_type: str
) -> str:
    ...
    if "foreignkeys" in ds_info["meta"]:
        dataset = get_action("recombinant_show")(
            ...
            {"dataset_type": dataset_type, "owner_org": owner_org},
        )
    ...
    snippet = (
        "pd_datatable.html"
        if config.get("ckanext.canada.enable_pd_datatable_editor")
        else "pd_datatable_depr.html"
    )
...
def cdts_asset(file_path: str, theme: Optional[bool] = True) -> str:
    return (
        CDTS_URI + "/" + CDTS_VERSION + ("/wet-boew" if theme else "/cdts") + file_path
    )
...
def recombinant_description_to_markup(text: str) -> Dict[str, Markup]:
    ...
    for i, part in enumerate(re.split(url_pattern, h.recombinant_language_text(text))):
        if i % 2:
            ...
            markup.extend(Markup("<br/>".join(escape(t) for t in part.split("\n"))))
...
def get_user_email(user_id: str) -> str:
    ...
    context = cast(
        Context, {"model": model, "session": model.Session, "keep_email": True}
    )
...
def organization_member_count(id: str) -> int:
    try:
        members = get_action("member_list")(
            cast(Context, {}), {"id": id, "object_type": "user", "include_total": True}
        )
    except ObjectNotFound:
...
def _build_flash_html_for_ga4(
    message: str, category: str, caller: str, allow_html: Optional[bool] = True
) -> str:
    """
...
def flash_notice(message: str, allow_html: Optional[bool] = True):
    ...
    """
    h.flash(
        _build_flash_html_for_ga4(
            message, "notice", _get_caller_info(inspect.stack()), allow_html=allow_html
        ),
        category="alert-info",
    )
...
def flash_error(message: str, allow_html: Optional[bool] = True):
    ...
    h.flash(
        _build_flash_html_for_ga4(
            message, "error", _get_caller_info(inspect.stack()), allow_html=allow_html
        ),
...
def flash_success(message: str, allow_html: Optional[bool] = True):
    ...
    h.flash(
        _build_flash_html_for_ga4(
            message, "success", _get_caller_info(inspect.stack()), allow_html=allow_html
        ),
...
def get_loader_status_badge(resource: Dict[str, Any]) -> Union[Markup, str]:
    ...
    if (
        resource.get("url_type") != "upload" and url_parts.netloc not in allowed_domains
    ) or not XLoaderFormats.is_it_an_xloader_format(resource.get("format")):
    ...
    elif xloader_job.get("status") in [
        "pending",
        "running",
        "running_but_viewable",
        "error",
    ]:
    ...
    title = (
        h.render_datetime(xloader_job.get("last_updated"), with_hours=True)
        if xloader_job.get("last_updated")
        else ""
    )
...
def get_resource_view(resource_view_id: str) -> Optional[Dict[str, Any]]:
    ...
    """
    try:
        return get_action("resource_view_show")({}, {"id": resource_view_id})
    except ObjectNotFound:
        return None
...
def split_piped_bilingual_field(
    field_text: Optional[str], client_lang: str
) -> Optional[str]:
...
def search_filter_pill_link_label(
    search_field: str, search_extras: Dict[str, Any]
) -> List[Any]:
    links_labels = []
    ...
    if search_field == "portal_type":
        ...
        preset_choices = (h.scheming_get_preset("canada_" + search_field) or {}).get(
            "choices", []
        )
    ...
    def remove_filter_button_link_label(
        _field: str, _value: str, s_extras: Dict[str, Any], _choices: List[Any]
    ):
    ...
    for value in g.fields_grouped[search_field]:
        if not isinstance(value, text_type):
            for v in value:
                links_labels.append(
                    remove_filter_button_link_label(
                        _field=search_field,
                        _value=v,
...
def operations_guide_link(stub: Optional[str] = None) -> str:
    ...
    try:
        ...
        guide_link = {
            "en": "https://open.canada.ca/en/registry-operations-guide",
...
def obfuscate_to_code_points(
    string: str, return_safe: bool = True
) -> Union[Markup, str]:
    """
    ...
    return Markup(obfuscated_string) if return_safe else obfuscated_string
...
def support_email_address(xml_encode: bool = True) -> Union[Markup, str]:
    return (
        ...
        if not xml_encode
        else obfuscate_to_code_points(config["ckanext.canada.support_email_address"])
    )
...
def mail_to_with_params(
    email_address: str, name: str, subject: str, body: str
) -> Markup:
    email = obfuscate_to_code_points(email_address, return_safe=False)
    if email_address == name:
        author = obfuscate_to_code_points(name, return_safe=False)
    ...
    html = Markup(
        '<a href="mailto:{0}?subject={2}&body={3}">{1}</a>'.format(
